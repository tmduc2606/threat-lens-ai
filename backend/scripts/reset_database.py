...
def upsert_index(source_type: str, source_key: str, title: str, summary: str, severity: str, score: float, tags: list[str], created_at=None, updated_at=None):
    existing = db.query(IntelIndex).filter(IntelIndex.source_type == source_type, IntelIndex.source_key == source_key).one_or_none()
    if existing:
        ...
        db.add(IntelIndex(source_type=source_type, source_key=source_key, title=title, summary=summary, severity=severity, score=score, tags=", ".join(tags), created_at=created_at, updated_at=updated_at))
...
try:
    ...
    # Step 3: Load OTX pulses
    print("Loading OTX pulses...")
    otx_path = data_dir / "1_otx_threat_intel.csv"
    if otx_path.exists():
        otx_df = normalize_columns(pd.read_csv(otx_path))
        ...
        db.commit()
        print(f"  Loaded {len(otx_df)} OTX pulses")
    else:
        print(f"  File not found: {otx_path}")

    # Step 4: Load CVEs
    print("Loading CVEs...")
    cve_path = data_dir / "2_cve_vulnerabilities.csv"
    if cve_path.exists():
        cves_df = normalize_columns(pd.read_csv(cve_path))
    ...
    dom_path = data_dir / "3_malicious_domains.csv"
    if dom_path.exists():
        domains_df = normalize_columns(pd.read_csv(dom_path))
        for _, row in domains_df.iterrows():
            ...
            domain = MaliciousDomain(
                ...
                creation_date=parse_date(row.get("creation_date")),
        ...
        print(f"  File not found: {dom_path}")

    # Step 6: Load IPs
    print("Loading IPs...")
    ip_path = data_dir / "4_malicious_ips.csv"
    if ip_path.exists():
        ips_df = normalize_columns(pd.read_csv(ip_path))
        ...
        db.commit()
        print(f"  Loaded {len(ips_df)} IPs")
    else:
        print(f"  File not found: {ip_path}")

    print("\nDatabase reset complete. All data loaded from data/.")
